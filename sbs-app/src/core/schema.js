...
export const APP_VERSION  = 'V0.3.0.66';
...
export function createPrimitiveNode(overrides = {}) {
  return {
    ...
    // Parameter-link group (V0.3.0.66). Primitives sharing a primLinkId share
...
export function createHardwareInstanceNode(overrides = {}) {
  return {
    ...
    //                         (washer j explodes to j·X, screw to
    //                         L + (W+1)·X). Default 20.
    //   repositionMs number — pre-insertion reposition time: how long the
    //                         screw takes to travel from its PREVIOUS-step
    //                         pose to the exploded staging pose before the
...
export function createColorPreset(overrides = {}) {
  return {
    ...
    ssrReflective:       false,     // V0.3.0.66: surface shows real SSR contact reflections
    flatMirror:          false,     // V0.3.0.66: flat faces of this colour get true planar mirrors
...
export function createCable(overrides = {}) {
  return {
    ...
    visible:      true,
    highlight:    false,
    flexible:     false,                  // V0.3.0.66: smooth spline through nodes (socketed ends
