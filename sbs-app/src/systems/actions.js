...
import { createAnimationPreset, createCameraView, createNode, createNoteNode, createNoteTemplate, createShapeTemplate, createShapeTemplateGroup, createFlatShapeNode, createPrimitiveNode, generateId, CAMERA_STATE_FIELDS, pickCameraView, pickCameraOrbit } from '../core/schema.js';
...
export async function commitTransformEditGlobalCtrl(nodeId) {
  ...
  _transformBatch = null;
  _msXf = null;                                   // suppress the deselect-time multi-step prompt
  ...
  const dOff = [
    ...
    (to.localOffset?.[2] || 0) - (from.localOffset?.[2] || 0),
  ];
  ...
  const choice = await chooseFromButtons(
    ...
    `Carry this ${what} of "${node.name || 'Object'}" to which steps?`,
    [
      { id: 'all',     label: 'All steps', primary: true },
      { id: 'earlier', label: 'Previous + this step' },
      { id: 'later',   label: 'Following + this step' },
      { id: 'this',    label: 'This step only' },
...
// 🎯 orbitPivot rides along (V0.3.2.234): without it, "Update step camera"
// would silently drop the step's animation orbit centre the next time the
// user pressed C. getCameraState only emits it when one is set, so a capture
// without one writes undefined — which JSON.stringify omits, leaving old
// files byte-identical. Camera TEMPLATES carry it too, deliberately: a
...
export function deleteCameraTemplate(templateId, replacement = null) {
  ...
  // The template's VIEW only (V0.3.4.26). Each step keeps its own orbit centre
  // and pull-out below — those describe the move into that step, not the
  // framing the template held.
  const tplCamSnapshot = pickCameraView(tpl);
  ...
  const nextSteps = allSteps.map(s => {
    ...
    return {
      ...
      snapshot: {
        ...(s.snapshot || {}),
        camera: { ...tplCamSnapshot, ...pickCameraOrbit(s.snapshot?.camera) },
      },
    };
  });
 ...
 *
 * An ABSOLUTE world point, like a 3D-anchored arrow's endpoint: bound to no
 * object and following none. It exists purely so the step-to-step camera
 * move orbits around it — exploring the scene by hand still orbits around
 * whatever the cursor's raycast hits, untouched.
 *
 * Stored in `snapshot.camera.orbitPivot`, so saving, copy-paste, project
 * import and the camera tween all carry it with the step for free.
 *
 * `worldPoint` = a THREE.Vector3 to set, or null to remove. Applies to the
...
export function setStepOrbitPivot(worldPoint, stepIds = null) {
  ...
  const nextSteps = allSteps.map(s => {
    ...
    if (pivot) cam.orbitPivot = pivot;
    else       delete cam.orbitPivot;
    n++;
  ...
  const apply = (list) => {
    ...
    if (act && ids.includes(act.id) && act.snapshot?.camera) {
      sceneCore.setOrbitPivot(Array.isArray(act.snapshot.camera.orbitPivot)
        ? new window.THREE.Vector3(...act.snapshot.camera.orbitPivot) : null);
    }
